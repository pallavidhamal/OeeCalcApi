 ...
 * userDetailsService; this.jwtAuthenticationFilter = jwtAuthenticationFilter; }
 * 
 * @Bean public SecurityFilterChain securityFilterChain(HttpSecurity
 * httpSecurity) throws Exception { httpSecurity .csrf(csrf -> csrf.disable())
 * .authorizeHttpRequests( request -> request
 * .requestMatchers("register","login").permitAll()
 * .anyRequest().authenticated() ) .httpBasic(Customizer.withDefaults())
 * .addFilterBefore(jwtAuthenticationFilter,
 * UsernamePasswordAuthenticationFilter.class); return httpSecurity.build(); }
 * 
 * 
 * //@Bean public UserDetailsService userDetailsService() { UserDetails shabbir
 * = User.withUsername("shabbir") .password("{noop}password") .roles("USER")
 * .build();
 * 
 * UserDetails nikhil = User.withUsername("nikhil") .password("{noop}nikhil")
 * .roles("USER") .build(); return new
 * InMemoryUserDetailsManager(shabbir,nikhil); }
 * 
